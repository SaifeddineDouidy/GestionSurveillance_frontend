...
const EnseignantsPage = () => {
  ...
  const [currentEnseignant, setCurrentEnseignant] = useState<Enseignant | null>(null);
...
const filteredEnseignants = enseignants.filter((ens) =>
  ...
  const handleAddEnseignant = async () => {
    try {
      const response = await axios.post("http://localhost:8088/api/enseignants", {
        ...newEnseignant,
  ...
  const handleModifyEnseignant = async () => {
    ...
    try {
      ...
      setEnseignants((prev) =>
        prev.map((ens) => (ens.id === currentEnseignant.id ? response.data : ens))
      );
  ...
  const handleDeleteEnseignant = async () => {
    ...
    try {
      await axios.delete(`http://localhost:8088/api/enseignants/${enseignantToDelete}`);
      setEnseignants((prev) => prev.filter((ens) => ens.id !== enseignantToDelete));
    ...
    <div className="p-8 bg-gray-50 min-h-screen">
      ...
      <div className="bg-white rounded-lg shadow p-6">
        ...
        <div className="space-y-1">
          <h1 className="text-2xl font-bold">Enseignants ({enseignants.length})</h1>
          <Link
              href="/departement"
            ...
            <tbody>
              {filteredEnseignants.map((enseignant) => (
                <tr key={enseignant.id} className="hover:bg-gray-100">
                  ...
                  <td className="px-4 py-2">{enseignant.dispense ? "Yes" : "No"}</td>
                  <td className="px-1 py-2 flex space-x-2">
                    <button
                      className="text-blue-500 hover:text-blue-700"
                      onClick={() => handlePrepareModifyEnseignant(enseignant)}
                    >
                      <FontAwesomeIcon icon={faEdit} />
                    </button>
                    <button
                      className="text-red-500 hover:text-red-700"
                      onClick={() => handlePrepareDeleteEnseignant(enseignant.id)}
                    >
      ...
      <Dialog open={isAddModalOpen} onOpenChange={setAddModalOpen}>
        <DialogContent>
          ...
          <Input
            ...
            onChange={(e) => setNewEnseignant({ ...newEnseignant, name: e.target.value })}
          />
          <Input
            placeholder="Email"
          ...
          <label className="flex items-center space-x-2">
            <input
              ...
              onChange={(e) =>
                setNewEnseignant({ ...newEnseignant, dispense: e.target.checked })
              }
            />
            <span>Dispense</span>
          </label>
