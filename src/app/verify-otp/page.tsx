...
export default function VerifyOtpPage() {
  ...
  const onSubmit: SubmitHandler<VerifyOtpInputs> = async (data) => {
    ...
    try {
      const response = await fetch(`http://localhost:8088/forgotPassword/verifyOTP/${data.otp}/${data.email}`, {
        method: "POST",
      ...
      <Card className="w-full max-w-md p-6 bg-white rounded-lg shadow-md">
        ...
        <CardContent>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div>
              <label className="block font-medium mb-2 text-gray-700">OTP:</label>
              <Input
                type="number"
